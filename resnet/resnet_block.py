...
class resnet_block(object):
    ...
    def bottleneck_block(x, oc, name, trainable=True, weight_decay=0.0001, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            w, h, ic = x.get_shape()
            weight = tf.get_variable(name + '-filter-1',
                                     shape=[oc['ksize'], oc['ksize'], ic, oc['channel']],
                                     initializer=tf.truncated_normal_initializer(stddev=0.01),
                                     trainable=trainable,
                                     regularizer=tf.contrib.layers.l2_regularizer(weight_decay))
            o = tf.nn.conv2d(x, filter=weight, strides=[1, oc['stride'], oc['stride'],1], padding='SAME', name='-conv2d-1')
            o = slim.batch_norm(o, is_training=trainable, fused=True, scope='batch-norm-1')
            o = tf.nn.relu(o, name='-relu-1')

            weight = tf.get_variable(name + '-filter-2',
                                     shape=[oc['ksize'], oc['ksize'],  o.get_shape()[-1], oc['channel']],
                                     initializer=tf.truncated_normal_initializer(stddev=0.01),
                                     trainable=trainable,
                                     regularizer=tf.contrib.layers.l2_regularizer(weight_decay))
            o = tf.nn.conv2d(o, filter=weight, strides=[1, oc['stride'], oc['stride'], 1], padding='SAME', name='-conv2d-2')
            o = slim.batch_norm(o, is_training=trainable, fused=True, scope='batch-norm-1')
    ...
    def bottleneck_block_11(x, oc, name, trainable=True, weight_decay=0.0001, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
            w, h, ic = x.get_shape()
            weight = tf.get_variable(name + 'filter-11-1',
            ...
            weight = tf.get_variable(name + '-filter-11-2',
