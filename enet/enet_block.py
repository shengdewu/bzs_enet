...
class enet_block(object):
    ...
    def initial_block(self, input, scope):
        ...
        output = tf.concat([bn, max_pool], axis=3, name=scope+'concat')
        return output
    ...
    def bottleneck_downsample(self, input, output_depth, filter_size, scope, projection=4, drop_prob=0.01, is_training=True):
        ...
        #first 1*1
        sub_net = slim.conv2d(input, project_depth, [2, 2], stride=2, scope=scope+'project_1')
        ...
        #second 1*1
        sub_net = slim.conv2d(sub_net, output_depth, [1, 1], stride=1, scope=scope+'project_2')
        sub_net = self.prebn(sub_net, is_training=is_training, scope=scope+'prebn3')
        ...
        main_net = tf.pad(main_net, paddings=padding, name=scope+'main_branch_pad')

        net = tf.add(main_net, sub_net)
        output = nn.prelu(net, scope=scope+'prelu2')
    ...
    def bottleneck_upsample(self, input, output_shape, filter_size, scope, pool_indices, projection=4, drop_prob=0.01, is_training=True):
        ...
        project_depth = int(input_shape[3] / projection)
        #first 1*1
        sub_net = slim.conv2d(input, project_depth, [1, 1], stride=1, scope=scope+'project_1')
        sub_net = self.prebn(sub_net, is_training=is_training, scope=scope+'prebn1')

        #conv
        filter_shape = [filter_size, filter_size, output_shape[-1], project_depth]
        filter_shape_tensor = tf.get_variable(shape=filter_shape, initializer=tf.constant_initializer, dtype=tf.float32, name=scope+'transpose_shape')
        sub_net = tf.nn.conv2d_transpose(sub_net, filter=filter_shape_tensor, output_shape=output_shape, strides=[1, 2, 2, 1], name=scope+'transpose')
        sub_net = self.prebn(sub_net, is_training=is_training, scope=scope+'prebn2')

        #second 1*1
        sub_net = slim.conv2d(sub_net, output_shape[-1], [1, 1], stride=1, scope=scope+'project_2')
        sub_net = self.prebn(sub_net, is_training=is_training, scope=scope+'prebn3')
        ...
        main_net = self.prebn(main_net, scope=scope + 'main_branch')
        main_net = nn.unpool(main_net, pool_indices, output_shape=output_shape, scope=scope + 'main_branch_unpool')
    ...
    def bottleneck(self, input, output_depth, filter_size, scope, btype='normal', dilation_rate=1, projection=4, drop_prob=0.01, is_training=True):
        ...
        if btype == 'decomposed':
            ...
            sub_net = slim.conv2d(sub_net, project_depth, decomposed_1, rate=dilation_rate, stride=1, scope=scope + 'conv1')
            sub_net = slim.conv2d(sub_net, project_depth, decomposed_2, rate=dilation_rate, stride=1, scope=scope + 'conv2')
        else:
